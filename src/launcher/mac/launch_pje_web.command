...
# ========================================
# LAUNCHER PJE WEB - Interface Desktop (macOS)
# ========================================
# Este arquivo pode ser colocado na area de trabalho
# e executado com duplo clique para iniciar a aplicacao
...
# Funcao para abrir o navegador automaticamente
open_browser() {
    sleep 3
...
cleanup() {
    print_status "Finalizando aplicacao..."
    if [ ! -z "$FLASK_PID" ]; then
        kill $FLASK_PID 2>/dev/null
    fi
    print_status "Aplicacao finalizada."
    exit 0
}
...
# Funcao para encontrar o diretorio do projeto
find_project_dir() {
    local current_dir="$1"
    local search_dirs=(
        "$current_dir"
...
# Obter o diretorio do script (onde o launcher esta)
SCRIPT_DIR="$(cd "$(dirname "${BASH_SOURCE[0]}")" && pwd)"

# Tentar encontrar o diretorio do projeto
print_status "Procurando projeto PDF-PJE..."
PROJECT_DIR=$(find_project_dir "$SCRIPT_DIR")

if [ -z "$PROJECT_DIR" ]; then
    print_error "Projeto PDF-PJE nao encontrado!"
    ...
    print_error ""
    print_error "Certifique-se de que o projeto PDF-PJE esta em um desses locais."
...
if ! command -v python3 &> /dev/null; then
    print_error "Python 3 nao esta instalado!"
...
if [ ! -d "venv" ]; then
    print_warning "Ambiente virtual nao encontrado!"
...
if ! pip show flask > /dev/null 2>&1; then
    ...
    if [ $? -ne 0 ]; then
        print_error "Falha ao instalar dependencias!"
        read -p "Pressione Enter para sair..."
        exit 1
    fi
fi

# Verificar se todas as dependencias estao instaladas
print_status "Verificando dependencias..."
pip install flask requests python-dotenv selenium webdriver-manager

# Iniciar aplicacao em background e abrir navegador
